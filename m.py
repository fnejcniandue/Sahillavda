...
# Function to handle the reply when free users run the /ritik command
...
def handle_ritik(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Check if the user has run the command before and is still within the cooldown period
            if user_id in ritik_cooldown and (datetime.datetime.now() - ritik_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown ❌. Please Wait 10sec Before Running The /ritik Command Again."
                bot.reply_to(message, response)
                return
            # Update the last time the user ran the command
            ritik_cooldown[user_id] = datetime.datetime.now()
        ...
        response = ("🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /ritik @RitikXyz099")

    bot.reply_to(message, response)


# Add /mylogs command to display logs recorded for ritik and website commands
...
💥 /ritik : Method For ritik Servers. 
